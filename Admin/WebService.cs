...
namespace Gator
    ...
    class WebService
        ...
        public bool Ban(string adname, string adpass, string usr)
            {
            MyWeb.MyWeb web = new MyWeb.MyWeb();
            NameValueCollection nvm = new NameValueCollection();
            nvm.Clear();
            ...
            while (resp == string.Empty)
                {
                resp = web.PostPage(urlban, nvm, 0, 0);
                if (resp == string.Empty)
                    System.Threading.Thread.Sleep(2000);
                }
            if (resp.Contains("BAN:SUCCESS"))
                return true;
            else
                return false;
            }
        public bool UnBan(string adname, string adpass, string usr)
            {
            MyWeb.MyWeb web = new MyWeb.MyWeb();
            NameValueCollection nvm = new NameValueCollection();
            nvm.Clear();
            ...
            if (resp.Contains("UNBAN:SUCCESS"))
                return true;
            else
                return false;
            }
