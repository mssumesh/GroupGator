...
namespace Gator
    ...
    class GGDisk
        ...
        public void AddFromGroup(string grpurl)
            {
            string buff = string.Empty;
            buff = Read(filefromgrps);
            if (!(buff.Contains("<GURL>" + grpurl + "</GURL>")))
                {
                Append("<GURL>" + grpurl + "</GURL>", filefromgrps);
                }
            }
        public void ClearFromGroups ( )
            {
            if ( File.Exists(filefromgrps))
                File.Delete( filefromgrps );
            }
